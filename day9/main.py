...
def load_heatmap(input_path: str) -> dict:
...
def find_mins(heatmap: dict) -> list[tuple[Any, Any]]:
    def is_min(x, y) -> bool:
        return (
...
def visit(x: int, y: int, visited: Set[Tuple[int, int]], heatmap: dict) -> Set[Tuple[int, int]]:
    visited.add((x, y))
...
def bassin_size(x: int, y: int, heatmap: dict) -> int:
